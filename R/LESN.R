...
bg.correct.shift <- function(object,baseline=0.25){
  ...
  pm(object) <-
      matrix(.C("R_shift_down", as.double(as.vector(pm(object))),
                as.double(baseline), as.integer(objsize[1]),
                as.integer(objsize[2]),
                PACKAGE="affyPLM")[[1]],objsize[1],objsize[2])
  object
}
...
bg.correct.stretch <- function(object,baseline=0.25,type=c("linear","exponential","loglinear","logexponential","loggaussian"),theta){
  ...
  pm(object) <- matrix(.C("R_stretch_down",
  as.double(as.vector(pm(object))), as.double(baseline),
  as.integer(objsize[1]), as.integer(objsize[2]), as.integer(ty),
  as.double(theta), PACKAGE="affyPLM")[[1]],objsize[1],objsize[2])
  object
}
