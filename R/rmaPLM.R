...
rmaPLM <- function(object,normalize=TRUE,background=TRUE,background.method="RMA.2",normalize.method="quantile",background.param = list(),normalize.param=list(),output.param=list(),model.param=list()){
  ...
  fit.results <- .Call("R_rmaPLMset_c",pm(object), mm(object), probeNames(object), ngenes, normalize, background, get.background.code(background.method), get.normalization.code(normalize.method),b.param,n.param,op.param,md.param) #, PACKAGE="AffyExtensions") 
